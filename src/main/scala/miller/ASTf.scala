...
object ASTf {
  ...
  object Program {
    ...
    implicit def op2f(o: AST.Op)(implicit st: ScopeStack): ASTf.Op = o match {
      case AST.Add(lhs, rhs, pos) =>   ASTf.Add(lhs, rhs, lhs.t intersect rhs.t intersect SetT(Set(TNumber, TString)), pos)
      case AST.Sub(lhs, rhs, pos) =>   ASTf.Sub(lhs, rhs, lhs.t intersect rhs.t intersect ConstT(TNumber), pos)
      case AST.Mul(lhs, rhs, pos) =>   ASTf.Mul(lhs, rhs, lhs.t intersect rhs.t intersect ConstT(TNumber), pos)
      case AST.Div(lhs, rhs, pos) =>   ASTf.Div(lhs, rhs, lhs.t intersect rhs.t intersect ConstT(TNumber), pos)
      case AST.Mod(lhs, rhs, pos) =>   ASTf.Mod(lhs, rhs, lhs.t intersect rhs.t intersect ConstT(TNumber), pos)
    ...
    implicit def value2f(v: AST.Value)(implicit st: ScopeStack): ASTf.Value = v match {
      ...
      case AST.JSCall(f, ps, pos)         => ASTf.JSCall(f, ps, f.t match {
        case VarT(n) => st.getType(n) match {
          ...
          case _ => NoInterErr(Set(st.getType(n), ConstT(TFunction(Seq(), AnyT))))
        }
      }, pos)
    }
    ...
    implicit def statement2f(s: AST.Statement)(implicit st: ScopeStack): ASTf.Statement = s match {
      case e: AST.Expr                 => e
      case AST.Return(v, pos)            => st.ret(v.t); ASTf.Return(v, pos)
      case AST.Declare(n, Some(e), pos)  => ASTf.Declare(st.declare(n, e.t), e, pos)
      case AST.Declare(n, None, pos)     => ASTf.Declare(st.declare(n), Undefined(pos), pos) //TODO
      case AST.Assign(n, e, pos)         =>
        val t = st.getId(n).map(st.getType).getOrElse(AnyT)
        expr2f(e)
        ASTf.Assign(st.getId(n), e, pos) // TODO check reassignment to different type
